...
public class TriangleReflection extends JFrame {
    private static final long serialVersionUID = 1L;

    int x1, y1, x2, y2, x3, y3;

    public TriangleReflection() {
        setTitle("Triangle Reflection");
        setSize(800, 600);
    ...
    private void initUI() {
        ...
        // Taking input for reflection axis (either 'x' or 'y')
        String reflectionAxis = JOptionPane.showInputDialog("Enter reflection axis ('x' or 'y'): ");

        // Applying reflection to triangle vertices using AffineTransform
        AffineTransform transform = new AffineTransform();

        if (reflectionAxis.equalsIgnoreCase("x")) {
            transform.setToScale(1, -1);
        } else if (reflectionAxis.equalsIgnoreCase("y")) {
            transform.setToScale(-1, 1);
        } else {
            throw new IllegalArgumentException("Invalid reflection axis. Use 'x' or 'y'.");
        }
